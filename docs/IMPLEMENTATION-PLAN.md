...
- **Panel-Cockpit-Redesign (F):**
  ...
  - **R1.3 — Kurven-Templates (Vorlagen)** ✅ **v0.8.0**: Werkzeug-Sektion mit Tabs (Vorlagen/Wolken/Ränder/Zeit); **Vorlagen** umgesetzt — Start/Dauer-Stepper + Paletten-Dropdown + 5 Formen (Plateau/Bolus/Glocke/Langer Sonnenaufgang/Natürlich) mit Mini-Kurven-Vorschau; Antippen legt die Form ins Fenster Start…Start+Dauer (lokal, dirty). Logik 1:1 aus dem App-Mockup übernommen. Wolken/Ränder/Zeit noch Platzhalter.
  - **R1.4 — restliche Tools** ✅ **v0.9.0**: **Wolken** (Zickzack 10–16 Uhr, Tiefe/Dichte-Stepper, Baseline abwechselnd abgesenkt), **Ränder** (0↔Wert-Kanten per Smoothstep weichzeichnen, Rampendauer), **Zeit** (Kurve ±15 min/±1 h verschieben, Endpunkte fix). Alle lokal+dirty, Logik 1:1 aus App-Mockup, per Harness numerisch verifiziert.
  - **R2 — Editor mit Schreiben** 🟡 **vorbereitet in v0.10.0, NOCH NICHT live getestet**: WS-Befehl `ati_straton/program/save` (`panel.py`) macht serverseitig Read-Modify-Write (nur die Nodes der Gruppe ersetzt; Node→`{time,value,type,index,color:{_id,name}}`) → `coordinator.async_apply_timelines()` → `put_data` **hinter `write_enabled` (Default AUS)**. Panel: „Speichern" nur aktiv wenn `write_enabled` (im Payload), sonst gesperrt mit Hinweis; sendet `{entry_id, timeline_id, nodes:[{time,value,color_id}]}`. Mit Mocks verifiziert (Payload korrekt, Guard beidseitig). **⚠️ ERSTER LIVE-WRITE steht noch aus — mit Rene, App getrennt, `write_enabled` in Optionen an, zuerst rechts-Gruppe/0 %.** Danach offen: Gesamtintensitäts-Slider, Play/Vorschau, Punkt-Drag, eigene Palette anlegen.
